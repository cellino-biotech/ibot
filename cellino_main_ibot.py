...
def train_ibot(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        # TODO: need to implement the following one
...
def train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss, data_loader,
    ...
    ncrops = args.global_crops_number #+ args.local_crops_number
    ...
    for it, batch_data in enumerate(metric_logger.log_every(data_loader, 10, header)):
        if isinstance(batch_data, dict): # to adapt cellino TFRecord
            ...
            masks = [batch_data[f'msk_{img_i}'] for img_i in range(ncrops)]
